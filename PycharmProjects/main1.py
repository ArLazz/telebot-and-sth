...
import telebot
import pb
from pb import data_of_messages
...
def cities_command(message):
    data_of_messages(message)
    bot.send_chat_action(message.chat.id, 'typing')
    temp = int(pb.load_weather(message.text)[0]['the_temp'])
    air_pressure = int(pb.load_weather(message.text)[0]['air_pressure'])
    wind_direction_compass = pb.load_weather(message.text)[0]['wind_direction_compass']
    wind_speed = int(pb.load_weather(message.text)[0]['wind_speed'])
    humidity = int(int(pb.load_weather(message.text)[0]['humidity']))
    weather_state_abbr = pb.load_weather(message.text)[0]['weather_state_abbr']
    weather_state_name = pb.load_weather(message.text)[0]['weather_state_name']
...
def valute_command(message):
    data_of_messages(message)
    bot.send_chat_action(message.chat.id, 'typing')
    exchange_data = pb.load_exchangerate()["Date"][:10]
    exchange_sell = pb.load_exchangerate()["Valute"]["{}".format(message.text[1:])]["Value"]
    exchange_name = pb.load_exchangerate()["Valute"]["{}".format(message.text[1:])]["Name"]
    exchange = 'Дата: {0}' \
               '\nВалюта: {1}' \
